...
export function stringifyChoiceOperator(operator: Operator) {
  ...
  const traverse: any = (operator: Operator) => {
    if (isLeaf(operator)) {
      ...
      if (Array.isArray((rest as any)[operatorName])) {
        const childOperators = (rest as any)[operatorName];
        return `(${childOperators.map(traverse).join(` ${operatorName} `)})`;
      } else {
        const childOperator = (rest as any)[operatorName];
...
export function getStates(stepFunction: StepFunction) {
  ...
  traverseStepFunction(stepFunction, (stateName, state) => {
    (states as any)[stateName] = oneLevelDeepClone(state);
  });
  return states;
}
