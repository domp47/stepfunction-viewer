import { Graph, json as graphLibJson } from "@dagrejs/graphlib";
...
import {
  ...
  getEdgeOptions,
  getMissingStyle,
} from "../../services/step-function/sfn-graph-styles";

const makeGroupName = () => `Group_${uuidv4()}`;
const makeNodeName = () => `Node_${uuidv4()}`;

const createMissingNodes = (g: Graph) => {
  const makeLabel = (edgePointer: any) => `${edgePointer} (Missing)`;
...
const serializeGraph = R.compose(JSON.stringify, graphLibJson.write);

const makeCluster = (g: Graph, state: State, parentClusterName: string | undefined) => {
...
export const buildGraph = (stepFunction: StepFunction) => {
  const g = new Graph({ compound: true, multigraph: true }).setGraph({}).setDefaultEdgeLabel(() => ({}));
